...
class StdImageField(ImageField):
    ...
    def _resize_image(self, filename, size):
        ...
        if img.size[WIDTH] > size['width'] or img.size[HEIGHT] > size['height']:
            ...
            try:
                #img.save(filename, optimize=1) fix save gif error
                img.save(filename)
            except IOError:
                img.save(filename)
    ...
    def contribute_to_class(self, cls, name):
        ...
        signals.pre_delete.connect(self._delete, sender=cls)

    def _delete(self, instance=None, **kwargs):
        """
        hack by hyliker@gmail.com
        do:delete the thumbnail
        """
        try:
            thumbnail = getattr(instance, self.name).thumbnail.path
            os.remove(thumbnail)
        except:
            pass
